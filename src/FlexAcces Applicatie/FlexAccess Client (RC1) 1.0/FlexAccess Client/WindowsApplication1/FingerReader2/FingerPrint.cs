...
namespace FlexAccess.FingerReader
{
  ...
  class FingerPrint
  {
    ...
    public FingerPrint(PictureBox pbPic, Person.Person person, TreeView trvFingers)
    {
      _pbFingerPrint = pbPic;
      _trvFingers = trvFingers;
    ...
    public void InitializeGrFinger(AxGrFingerXLib.AxGrFingerXCtrl grfingerx)
    {
      ...
      // Maken van een nieuw raw afbeelding
      _raw = new TRawImage();

      /** Stoppen met uitlezen **/
      //_grfingerx.Finalize();
    ...
    // Toevoegen van een vingerprint template naar de database
    public bool saveTemplate()
    {
      byte[] template = SerializeObject(_tpt);
      return _DB.saveTemplate(template, _person, _flcID);
    }
    ...
    public Person.Person Identify(ref int score)
    {
      GRConstants result;
      ...
      foreach (FlexAccessDataSet.module_fingerprintRow fingerPrintRow in fingerPrintData)
      {
        ...
        if (result == GRConstants.GR_MATCH)
        {
          ...
          try
          {
            Person.Person personFinger = new Person.Person();
    ...
    public void setFingerLocationID(int flcID)
    {
      this._flcID = flcID;
    }

    public int getFingerLocationID()
    {
      return this._flcID;
    }

    public Boolean deleteFinger()
    {
      return _DB.deleteTemplate(_person, _flcID);
    }
    ...
    public void markUnknownFingers()
    {
      FlexAccessDataSet.module_fingerprintDataTable fingerPrintData;
      fingerPrintData = _DB.getUnknownFingers(_person);

      /** Data uitlezen */
      foreach (FlexAccessDataSet.module_fingerprintRow fingerPrintRow in fingerPrintData) {
        //_trvFingers.Nodes["ndeFinger" + fingerPrintRow.flc_id].Text = "Hoi";
      }
    }
  }
}
